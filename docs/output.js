...
function outputMaterialList(ulObj, titleObj, shipTitle, classId, shipId) {
  $.getJSON("equipments.json" , function(data) {
    ...
    for(var i = 0; i < data.length; i++) {
      ...
      var listStyle = '<li id="i-'+ id + '" class="list_parent"/>';
      if (data[i].title.indexOf('その他') == -1) {
        subObj = $(listStyle).html(LIST_MARKOR + label + '<a href="https://akashi-list.me/#w' + data[i].id + '" title="「明石の工廠早見表」装備ページ" target="_blank" rel="noopener">' + data[i].title + '</a>'
        ...
        subObj = $(listStyle).html(LIST_MARKOR + label + data[i].title);
      }
      var isBonusFound = false;
      for(var j = 0; j < data[i].bonus.length; j++) {
        for(var k = 0; k < data[i].bonus[j].items.length; k++) {
          var subListStyle = '<li id="i-'+ id + '-' + j + '" class="list_parent"/>';
          if (data[i].bonus[j].items[k].ship_class == shipId) {
            subObj.append($('<ul/>')
              .append($(subListStyle).append(LIST_MARKOR + data[i].bonus[j].synergy)
                .append($('<ul/>')
